...
export default function Section02Introduction() {
  ...
  return (
    <section id="introduction" className="w-full bg-[#FFFFFF] text-[#111827] py-16 sm:py-24 lg:py-32 relative overflow-hidden border-b border-[#FFD978]/30 font-sans">
      {/* Subtle Background Accent Glow */}
      <div className="absolute top-1/3 left-0 w-96 h-96 bg-[radial-gradient(ellipse_at_center,rgba(244,185,66,0.06),transparent_70%)] pointer-events-none z-0" />
      ...
      <div className="lg:hidden w-full max-w-[640px] md:max-w-[768px] mx-auto px-5 sm:px-6 relative z-10 flex flex-col space-y-7">
        ...
        <div className="grid grid-cols-2 gap-6 py-6 border-t border-b border-[#FFD978]/30">
          {firmStats.map((stat, idx) => (
            <div key={idx} className="space-y-1">
              <span className="font-serif font-bold text-3xl sm:text-4xl text-[#1F3A8A] leading-none block">
                {stat.value}
              </span>
              <span className="font-mono text-[10px] sm:text-xs font-bold text-[#4B5563] uppercase tracking-wider block">
                {stat.label}
              </span>
            </div>
          ))}
      ...
      <div className="hidden lg:block w-full max-w-[1440px] mx-auto px-10 xl:px-16 relative z-10">
        <div className="grid grid-cols-12 gap-12 xl:gap-16 items-start">
          ...
          <div className="lg:col-span-3 xl:col-span-3 flex flex-col justify-start pr-8 border-r border-[#FFD978]/30 select-none">
            ...
            <div className="divide-y divide-[#FFD978]/30">
              {firmStats.map((stat, idx) => (
                <div key={idx} className="py-7 first:pt-0 last:pb-0 space-y-1.5">
                  <span className="font-serif font-bold text-5xl lg:text-[56px] xl:text-[64px] leading-none text-[#1F3A8A] block tracking-tight">
                    {stat.value}
                  </span>
                  <span className="font-mono text-xs font-bold uppercase tracking-[0.2em] text-[#4B5563] block">
                    {stat.label}
                  </span>
                </div>
              ))}
